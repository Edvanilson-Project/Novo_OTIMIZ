...
**Data:** 2026-05-24  
**URL testada:** http://localhost:3005/operations/planner  
**API testada:** http://localhost:3001/api/v1/operations/latest-schedule

---

## EXECUCAO RUNTIME CODEx - 2026-05-24 13:58

### Ambiente usado nesta rodada

- Frontend validado: `http://localhost:3005`
- Backend validado: `http://localhost:3001`
- Autenticacao E2E: `admin@empresa.com / admin123`

### Evidencia objetiva coletada

- Playwright smoke do planner: `5 passed`
- Playwright selectors/Gantt: `4 passed`
- Login E2E autenticado com redirecionamento para `/dashboard`
- `GET /api/v1/operations/latest-schedule`: schedule `id=4`, `status=completed`
- KPI backend: `2 veiculos`, `10 viagens`, `totalCost=3628.40`, `hardIssues=0`, `softIssues=0`

### Bug real encontrado nesta execucao

- `BUG-PLANNER-CORS-01`:
  frontend em porta local alternativa (`3003`) falhava no login porque a allowlist
  de CORS/WebSocket do backend cobria `3000` e `3005`, mas nao cobria outras portas
  de dev usadas na pratica.

### Correcao aplicada

- Arquivo: `backend/src/main.ts`
- Arquivo: `backend/src/modules/operations/optimization.gateway.ts`
- Ajuste: allowlist de dev expandida para `localhost` e `127.0.0.1` nas portas
  `3000` a `3005`

### Reteste executado

- Runtime real do Planejador em `http://localhost:3005`
- Login E2E OK
- Pagina do planner OK
- Dropdown de algoritmo OK
- Dropdown de qualidade operacional OK
- Aba Gantt OK

### Reteste ainda pendente

- Reiniciar o backend principal na porta usada pela UI alternativa e confirmar que o
  login passa tambem em `3003` com o processo corrigido em runtime.
