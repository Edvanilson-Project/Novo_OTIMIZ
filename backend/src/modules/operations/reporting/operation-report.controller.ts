...
export class OperationReportController {
  ...
  @ApiOperation({
    summary: 'Gerar relatório de operação',
    description: 'Computa KPIs, alertas e recomendações para a escala.',
  })
  @ApiParam({ name: 'scheduleId', type: Number })
  @ApiResponse({
    status: 201,
    description: 'Relatório gerado com KPIs e alertas.',
  })
  ...
  @ApiOperation({
    summary: 'Histórico de relatórios de otimização',
    description: 'Retorna runs de OptimizationRun dos últimos N dias.',
  })
  @ApiParam({ name: 'scheduleId', type: Number })
  @ApiQuery({ name: 'days', required: false, example: 30 })
  @ApiResponse({
    status: 200,
    description: 'Lista de runs históricos com métricas.',
  })
  async getHistoricalReports(
    @Param('scheduleId', ParseIntPipe) scheduleId: number,
    @Query('days', new ParseIntPipe({ optional: true })) days: number = 30,
  ) {
    return this.reportGenerator.getHistoricalReports(
      scheduleId,
      this.getCompanyId(),
      days,
    );
  ...
  @ApiOperation({
    summary: 'Comparar relatórios por período',
    description: 'Compara métricas entre duas datas ISO.',
  })
  @ApiParam({ name: 'scheduleId', type: Number })
  ...
  @ApiResponse({
    status: 200,
    description: 'Delta de KPIs entre os dois períodos.',
  })
  ...
  async compareReports(
    ...
    if (!startDate || isNaN(start.getTime())) {
      throw new BadRequestException(
        'startDate inválida ou ausente (formato ISO esperado)',
      );
    }
    if (!endDate || isNaN(end.getTime())) {
      throw new BadRequestException(
        'endDate inválida ou ausente (formato ISO esperado)',
      );
    }
  ...
  async exportPDF(
    ...
    const pdf = await this.reportGenerator.generatePDFReport(
      scheduleId,
      this.getCompanyId(),
    );
  ...
  @ApiOperation({
    summary: 'Estatísticas de jornadas',
    description: 'Retorna Gini, P5/P95, violações e custo por jornada.',
  })
  @ApiParam({ name: 'scheduleId', type: Number })
  @ApiResponse({
    status: 200,
    description: 'Resumo e detalhe por jornada com indicadores de equidade.',
  })
  ...
  async exportExcel(
    ...
    const excel = await this.reportGenerator.generateExcelReport(
      scheduleId,
      this.getCompanyId(),
    );
    res.setHeader(
