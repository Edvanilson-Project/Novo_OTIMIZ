...
export function normalizeLegacyCompanyParameters<T extends Partial<CompanyParameters> | null | undefined>(
  params: T,
): { normalized: T; changed: boolean } {
  if (!params) {
  ...
  for (const field of FRACTION_PERCENT_FIELDS) {
    ...
    if (typeof currentValue !== 'number' || !Number.isFinite(currentValue) || currentValue <= 1) {
      continue;
    }
