...
def test_vcsp_fallback_logic():
    ...
    trips_large = [
        Trip(id=i, line_id=1, start_time=480 + i, end_time=540 + i, origin_id=1, destination_id=2) for i in range(10)
    ]
...
def test_rostering_fallback_logic():
    ...
    try:
        res = solver.solve(operators, duties, rules=[], cct_params={"rostering_timeout_seconds": 0.001})
        if res.meta.get("fallback_used"):
...
def test_mcnf_redirection():
    ...
    trips = [
        Trip(id=i, line_id=1, start_time=480 + i, end_time=540 + i, origin_id=1, destination_id=2) for i in range(900)
    ]
    vt = [VehicleType(id=1, name="Bus", passenger_capacity=40)]
...
def test_optimizer_service_dispatch():
    ...
    try:
        res = svc._dispatch(AlgorithmType.GREEDY, trips, vt, 1, {}, {}, {}, 30.0)
