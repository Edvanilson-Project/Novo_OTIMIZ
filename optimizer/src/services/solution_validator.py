...
class SolutionValidator:
                ...
                if end1 and start2 and end1 > start2:
                    errors.append(
                        ValidationError(
                            ...
                            detail=f"Trip {trip1.get('tripId')} ends at {end1}, Trip {trip2.get('tripId')} starts at {start2}",  # noqa: E501
                        )
                    )
        return errors
    ...
    def _check_meal_break_position(
        self, duties: Sequence[Dict[str, Any]], params: Dict[str, Any]
    ) -> List[ValidationError]:
        return []

    def _calculate_stats(
        self, blocks: Sequence[Dict[str, Any]], duties: Sequence[Dict[str, Any]], trips: Sequence[Dict[str, Any]]
    ) -> Dict[str, Any]:
        allocated_trips = set()
        for block in blocks:
        ...
        total_operator_hours = sum(
            (d.get("end_time", 0) - d.get("start_time", 0)) / 60
            for d in duties
                    ...
                    reasons.append(
                        {
                            ...
                            "detail": f"Last vehicle trip ends at {last_end}, next trip starts at {trip_start} (gap {gap}min < {tolerance}min)",  # noqa: E501
                            "severity": "HIGH",
                        }
                    )
...
class MealBreakValidator:
    ...
    def validate_meal_break_position(duties, params):
        ...
        """
        errors = []
        params.get("meal_break_minutes", 60)
                    ...
                    errors.append(
                        {
                            ...
                            "severity": "WARNING",
                            "duty_id": duty_id,
                            "detail": "Duty > 6h but no meal_break_time recorded",
                    ...
                    errors.append(
                        {
                            ...
                            "vehicle_id": vehicle_id,
                            "trip_ids": [trip1.get("tripId"), trip2.get("tripId")],
                            "detail": f"Real deadhead {required_deadhead}min + 5min tolerance > available gap {available_gap}min",  # noqa: E501
