...
from .domain.models import Trip, VehicleType, AlgorithmType, OptimizationResult
from .services.optimizer_service import OptimizerService
from .api.schemas import OptimizationParametersDTO, TripInput, VehicleTypeInput
...
def create_mock_data() -> Tuple[List[Trip], List[VehicleType]]:
    ...
    for t in trips:
        ...
        if t.id == 3: t.is_holiday = True

    vt = [VehicleType(id=1, name="Standard", passenger_capacity=80, cost_per_km=1.0, fixed_cost=1000.0)]
    return trips, vt

def run_test(service: OptimizerService, name: str, trips: List[Trip], v_types: List[VehicleType], params: Dict[str, Any]) -> Dict[str, Any]:
...
def verify_all():
    ...
    test_cases = [
        ("dynamic_rules", [{"condition": {"field": "is_holiday", "op": "==", "value": True}, "action": {"target": "work_cost", "type": "multiply", "value": 5.0}}], "Dynamic Rule: Holiday multiplier"),
    ...
    for param, value, rationale in test_cases:
        ...
        if res.get("error"):
            ...
            diff = False
            if param == "dynamic_rules" and res["work_cost"] > baseline["work_cost"]: diff = True
            elif param == "nocturnal_start_hour" and res["nocturnal_extra"] > baseline["nocturnal_extra"]: diff = True
            elif param == "pullout_minutes" and res["spread"] > baseline["spread"]: diff = True
            elif param == "holiday_extra_pct" and res["holiday_extra"] > baseline["holiday_extra"]: diff = True
            elif param == "nocturnal_extra_pct" and res["nocturnal_extra"] > baseline["nocturnal_extra"]: diff = True
