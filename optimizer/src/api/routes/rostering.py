...
async def run_nominal_rostering(body: NominalRosteringRequest):
    ...
    """
    logger.info("nominal_rostering_request: operators=%d, duties=%d", len(body.operators), len(body.duties))

    try:
        # ── 1. Reconstrução do Domínio ──────────────────────────────────────
        operators = [
            OperatorProfile(id=op.id, name=op.name, cp=op.cp, last_shift_end=op.last_shift_end, metadata=op.metadata)
            for op in body.operators
        ]

        rules = [
            RosteringRule(rule_id=r.rule_id, type=RuleType(r.type), weight=r.weight, meta=r.meta) for r in body.rules
        ]
        ...
        solution = solver.solve(
            operators=operators, duties=duties, rules=rules, inter_shift_rest_minutes=body.inter_shift_rest_minutes
        )
        ...
        raise HTTPException(status_code=500, detail=f"Falha interna no motor de Rostering: {str(exc)}")
...
async def run_weekly_rostering(body: WeeklyRosteringRequest):
    ...
    try:
        ...
        daily_duties: dict = {int(day_idx): list(duty_list) for day_idx, duty_list in body.daily_duties.items()}
