...
def create_complex_mock() -> Tuple[List[Trip], List[VehicleType]]:
    ...
    t3 = Trip(
        ...
        destination_id=2,
        distance_km=10.0,
        mid_trip_relief_point_id=99,
        mid_trip_relief_offset_minutes=50,
    )
...
def test_single_line_duty():
    ...
    service.run(
        trips=[trips[0], trips[1]], vehicle_types=vt, algorithm=AlgorithmType.GREEDY, optimization_params=params_allow
    )
    ...
    try:
        service.run(
            ...
            optimization_params=params_restrict,
        )
...
def test_relief_points():
    ...
    res = service.run(
        trips=[trips[2]], vehicle_types=vt, algorithm=AlgorithmType.GREEDY, optimization_params=params_relief
    )
...
def test_enforce_same_depot():
    ...
    t1 = Trip(
        id=1, line_id=101, start_time=480, end_time=600, origin_id=1, destination_id=2, distance_km=10.0, depot_id=1
    )
    t2 = Trip(
        id=2, line_id=101, start_time=720, end_time=840, origin_id=2, destination_id=1, distance_km=10.0, depot_id=2
    )
...
def test_inter_shift_rest():
    ...
    params = {"inter_shift_rest_minutes": 660, "max_shift_minutes": 720, "max_driving_minutes": 600, "apply_cct": True}
    res = service.run(trips=[t1, t2], vehicle_types=vt, algorithm=AlgorithmType.GREEDY, optimization_params=params)
